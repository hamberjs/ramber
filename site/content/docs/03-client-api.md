...
Programmatically prefetches the given page, which means a) ensuring that the code for the page is loaded, and b) calling the page's `preload` method with the appropriate options. This is the same behaviour that Ramber triggers when the user taps or mouses over an `<a>` element with [ramber:prefetch](docs#ramber_prefetch).
