...
import { Handler, RamberRequest, RamberResponse, build_dir, dev, manifest } from '@ramber/internal/manifest-server';
...
export default function middleware(opts: {
	session?: (req: RamberRequest, res: RamberResponse) => any,
	ignore?: IgnoreValue
} = {}) {
	...
	return compose_handlers(ignore, [
		(req: RamberRequest, res: RamberResponse, next: () => void) => {
...
export function compose_handlers(ignore: IgnoreValue, handlers: Handler[]): Handler {
	...
	function nth_handler(n: number, req: RamberRequest, res: RamberResponse, next: () => void) {
		if (n >= total) {
			return next();
		}
...
export function serve({ prefix, pathname, cache_control }: {
	...
	const filter = pathname
		? (req: RamberRequest) => req.path === pathname
		: (req: RamberRequest) => req.path.startsWith(prefix);
	...
	return (req: RamberRequest, res: RamberResponse, next: () => void) => {
		if (filter(req)) {
