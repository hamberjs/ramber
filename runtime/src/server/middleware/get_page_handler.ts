...
import {
    ...
    dev,
    src_dir
} from '@ramber/internal/manifest-server';
import App from '@ramber/internal/App.hamber';
import { PageContext, PreloadResult } from '@ramber/common';
import detectClientOnlyReferences from './detect_client_only_references';
import { TLSSocket } from 'tls';

export function get_page_handler(
	manifest: Manifest,
	session_getter: (req: RamberRequest, res: RamberResponse) => Promise<any>
) {
	...
	function bail(res: RamberResponse, err: Error | string) {
		console.error(err);
	...
	function handle_error(req: RamberRequest, res: RamberResponse, statusCode: number, error: Error | string) {
		handle_page({
			pattern: null,
			parts: [
		...
		try {
			...
			const serialized = {
				preloaded: `[${preloaded.map(data => try_serialize(data, err => {
					...
					console.warn('The client will re-render over the server-rendered page fresh instead of continuing where it left off. See https://ramberjs.web.app/docs#Return_value for more information');
				})).join(',')}]`,
			...
			const body = template()
				.replace('%ramber.base%', () => `<base href="${req.baseUrl}/">`)
				.replace('%ramber.scripts%', () => `<script${nonce_attr}>${script}</script>`)
				.replace('%ramber.html%', () => html)
				.replace('%ramber.head%', () => head)
				.replace('%ramber.styles%', () => styles)
				.replace(/%ramber\.cspnonce%/g, () => nonce_value);
