import { writable } from 'hamber/store.mjs';
import App from '@ramber/internal/App.hamber';
import { stores } from '@ramber/internal/shared';
import { Root, root_preload, ErrorComponent, ignore, components, routes } from '@ramber/internal/manifest-client';
import {
	Target,
	ScrollPosition,
	Component,
...
declare const __RAMBER__;
export const initial_data = typeof __RAMBER__ !== 'undefined' && __RAMBER__;
...
async function render(redirect: Redirect, branch: any[], props: any, page: Page) {
	...
	if (root_component) {
		...
		const start = document.querySelector('#ramber-head-start');
		const end = document.querySelector('#ramber-head-end');
