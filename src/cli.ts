...
const prog = sade('ramber').version(pkg.version);

if (process.argv[2] === 'start') {
	// remove this in a future version
	console.error(colors.bold().red('"ramber start" has been removed'));
...
prog.command('dev')
	...
	.option('--output', 'Ramber intermediate file output directory', 'src/node_modules/@ramber')
	.option('--build-dir', 'Development build directory', '__ramber__/dev')
	.option('--ext', 'Custom Route Extension', '.hamber .html')
	.action(async (opts: {
		port: number;
		open: boolean;
		'dev-port': number;
...
prog.command('build [dest]')
	...
	.option('--routes', 'Routes directory', 'src/routes')
	.option('--output', 'Ramber intermediate file output directory', 'src/node_modules/@ramber')
	.option('--ext', 'Custom page route extensions (space separated)', '.hamber .html')
	.example('build custom-dir -p 4567')
	.action(async (dest = '__ramber__/build', opts: {
		port: string;
		legacy: boolean;
		bundler?: 'rollup' | 'webpack';
		cwd: string;
		...
		try {
			...
			fs.writeFileSync(launcher, `
				// generated by ramber build at ${new Date().toISOString()}
...
prog.command('export [dest]')
	...
	.option('--static', 'Static files directory', 'static')
	.option('--output', 'Ramber intermediate file output directory', 'src/node_modules/@ramber')
	.option('--build-dir', 'Intermediate build directory', '__ramber__/build')
	.option('--ext', 'Custom page route extensions (space separated)', '.hamber .html')
	.option('--entry', 'Custom entry points (space separated)', '/')
	.action(async (dest = '__ramber__/export', opts: {
		build: boolean;
		legacy: boolean;
		bundler?: 'rollup' | 'webpack';
		basepath?: string;
