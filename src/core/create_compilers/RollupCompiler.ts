...
export default class RollupCompiler {
	...
	async get_config(mod: any) {
		...
		const css_injection: Plugin = {
			...
			renderDynamicImport({ targetModuleId }) {
				if (targetModuleId) {
					return {
						...
						right: `), ___RAMBER_CSS_INJECTION___${Buffer.from(targetModuleId).toString('hex')}___]).then(function(x) { return x[0]; })`
					};
				} else {
					return {
						left: 'import(',
			...
			async generateBundle(this: PluginContext, options: NormalizedOutputOptions, bundle: OutputBundle): Promise<void> {
				...
				for (const name in bundle) {
					...
					if (chunk.code) {
						chunk.code = chunk.code.replace(/___RAMBER_CSS_INJECTION___([0-9a-f]+)___/g, (m, id) => {
		 ...
		 */
		const ramber_internal: Plugin = {
			name: 'ramber-internal',
			async generateBundle(this: PluginContext, options: NormalizedOutputOptions, bundle: OutputBundle): Promise<void> {
		...
		mod.plugins.push(ramber_internal);
