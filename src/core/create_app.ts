...
export function create_app({
	...
	write_if_changed(`${output}/internal/App.hamber`, app);
}
...
export function create_serviceworker_manifest({ manifest_data, output, client_files, static_files }: {
	...
	if (fs.existsSync(static_files)) {
		...
		if (fs.existsSync('assets')) {
			throw new Error(`As of Ramber 0.21, the assets/ directory should become static/`);
		}
	}

	let code = `
		// This file is generated by Ramber — do not edit it!
...
function generate_client_manifest(
	...
	const components = `[
		${manifest_data.components.map((component, i) => {
			...
			return `{
					...
					css: "__RAMBER_CSS_PLACEHOLDER:${stringify(component.file, false)}__"
				}`;
		}).join(',\n\t\t\t\t')}
	]`.replace(/^\t/gm, '');
	...
	return `
		...
		${dev ? `if (typeof window !== 'undefined') {
			import(${stringify(posixify(path.resolve(__dirname, '../ramber-dev-client.js')))}).then(client => {
...
function generate_app(manifest_data: ManifestData, path_to_routes: string) {
	...
	let pyramid = `<hamber:component this="{level${l}.component}" {...level${l}.props}/>`;

	while (l-- > 1) {
		pyramid = `
			<hamber:component this="{level${l}.component}" segment="{segments[${l}]}" {...level${l}.props}>
				{#if level${l + 1}}
					${pyramid.replace(/\n/g, '\n\t\t\t\t\t')}
				{/if}
			</hamber:component>
...
function get_file(path_to_routes: string, component: PageComponent) {
	if (component.default) return `./${component.type}.hamber`;
